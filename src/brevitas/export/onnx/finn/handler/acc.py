...
from brevitas.nn import QuantAvgPool2d, QuantAvgPool3d, QuantAdaptiveAvgPool3d
...
class FINNQuantAvgPool3dHandler(FINNQuantIOHandler):
    handled_layer = (QuantAvgPool3d, QuantAdaptiveAvgPool3d)

    @staticmethod
    def quant_output_shape(module: QuantAvgPool3d):
        shape = FINNQuantIOHandler.quant_output_shape(module)
        if shape is None:
            raise RuntimeError("Caching of output shapes is required to export QuantAvgPool3d")
        return shape

    @staticmethod
    def quant_input_bit_width(module: QuantAvgPool3d):
        bit_width = FINNQuantIOHandler.quant_input_bit_width_tensor(module)
        if bit_width is None:
            raise RuntimeError("Caching of input bit width is required to export QuantAvgPool3d")
        return int(bit_width.item())

    @staticmethod
    def quant_output_bit_width(module: QuantAvgPool3d):
        bit_width = FINNQuantIOHandler.quant_output_bit_width_tensor(module)
        if bit_width is None:
            raise RuntimeError("Caching of output bit width is required to export QuantAvgPool3d")
        return int(bit_width.item())

    @staticmethod
    def quant_input_signed(module: QuantAvgPool3d) -> int:
        signed = FINNQuantIOHandler.quant_input_signed(module)
        if signed is None:
            raise RuntimeError("Output sign of QuantAvgPool3d is malformed")
        return int(signed)
    ...
    def symbolic_execution(self, inp: Tensor):
        ret = QuantAvgPool3dFn.apply(inp, *self.symbolic_kwargs.values())
        return ret